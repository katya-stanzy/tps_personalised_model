...
# I updated this function such that the interpolation happens first and thresholding after
...
def refine_ground_reaction_wrench_ks(data_table, label_triplet, stance_threshold,
                                  tau, debug=True):
   ...
   # remove information when the foot is not touching the ground
    ...
    if i0 and ifin:
        start = max([0, i0 - 60])
        end = min(ifin+60, len(f_l))
        
        for frame in range(start,i0):   
            p_l[frame,0] = p_l[i0,0]; p_l[frame,1] = p_l[i0,1]; p_l[frame,2] = p_l[i0,2]
        
        for frame in range(ifin,end):
            p_l[frame,0] = p_l[ifin-1,0]; p_l[frame,1] = p_l[ifin-1,1]; p_l[frame,2] = p_l[ifin-1,2]
